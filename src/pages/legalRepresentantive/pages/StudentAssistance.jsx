...
const StudentAssistancePage = () => {
  ...
  const fetchAttendance = async () => {
    try {
      const res = await fetch(`http://localhost:3000/api/legal-representatives/${roleId}/estudiantes/${selectedStudent}/asistencias`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
              ...
              <div key={record.id} className="attendance-card">
                <div className="attendance-header">
                  <div className="student-info">
                    <h3 className="student-name">{record.studentName || selectedStudent}</h3>
                    <p className="student-grade">{record.grade || 'Grado desconocido'}</p>
                  </div>
                ...
                <div className="attendance-details">
                  <div className="time-info">
                    <div className="time-item">
                      ...
                      <span className="time-value">{record.entryTime || '--:--'}</span>
                    </div>
                    <div className="time-item">
                      <span className="time-label">Salida:</span>
                      <span className="time-value">{record.exitTime || '--:--'}</span>
                    </div>
                  </div>
